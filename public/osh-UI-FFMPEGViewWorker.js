...
if (!self.av_frame) alert("Could not allocate video frame");
...
self.onmessage = function(e) {
  var data = e.data;
  //console.log("show:", e.data.show);
  const { show } = e.data;

  var decodedFrame = innerWorkerDecode(
    data.pktSize,
    new Uint8Array(data.pktData, data.byteOffset, data.pktSize)
  );
  if (typeof decodedFrame != "undefined") {
    self.postMessage({ decodedFrame, show }, [
...
function innerWorkerDecode(pktSize, pktData) {
  ...
  // decode next frame
  var len = _avcodec_decode_video2(
    self.av_ctx,
    self.av_frame,
    self.got_frame,
    self.av_pkt
  ...
  return {
    ...
    frameYData: new Uint8Array(
      Module.HEAPU8.buffer.slice(
        frameYDataPtr,
        frameYDataPtr + frame_width * frame_height
      )
    ),
    frameUData: new Uint8Array(
      Module.HEAPU8.buffer.slice(
        frameUDataPtr,
        frameUDataPtr + ((frame_width / 2) * frame_height) / 2
      )
    ),
    frameVData: new Uint8Array(
      Module.HEAPU8.buffer.slice(
        frameVDataPtr,
